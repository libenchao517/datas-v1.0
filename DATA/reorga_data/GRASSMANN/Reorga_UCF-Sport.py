################################################################################
## 本代码用于将UCF-Sport数据集建模至Grassmann空间
################################################################################
## 导入模块
import os
import re
...
for k, cate in enumerate(categories):
    ...
    for object in objects:
        # object_path = os.path.join(cate_path, object, "jpeg")
        object_path = os.path.join(cate_path, object)
        files = [item for item in os.listdir(object_path) if re.match(r".*.jpg", item)]
