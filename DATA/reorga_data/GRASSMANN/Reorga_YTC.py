################################################################################
# 本文件用于整理YouTuBe Celebrity数据集至Grassmann空间
################################################################################
# 导入模块
import os
import re
...
subjects = np.unique([item[12:-4] for item in files])
for k, subject in enumerate(subjects):
    avi_files = [item for item in files if re.match(rf".*{subject}.avi", item)]
    target_dict[subject] = k
    for avi in avi_files:
        avi_path = os.path.join(root_path, avi)
        S = convert_video(avi_path)
        data.append(S)
        target.append(target_dict.get(subject))
################################################################################
# 建模导Grassmann流形
images = data
for p in range(5, 16):
