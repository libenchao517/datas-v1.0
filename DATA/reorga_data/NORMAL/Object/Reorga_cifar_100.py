...
################################################################################
# 定义基本变量
root_path = "cifar-100-python/cifar-100-python"
data_name = "CIFAR-100"
pixel_matrix = []
labels = [str(item) for item in range(100)]
...
file_path = os.path.join(root_path, "train")
bin = unpickle(file_path)
data, target = np.array(bin[b"data"]), np.array(bin[b"fine_labels"])

file_path = os.path.join(root_path, "test")
bin = unpickle(file_path)
data_, target_ = np.array(bin[b"data"]), np.array(bin[b"fine_labels"])
